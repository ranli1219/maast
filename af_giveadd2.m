function igpdata = af_giveadd2(t, igpdata, wrsdata, satdata, wrs2satdata, ...
                              truth_data)
%*************************************************************************
...
global GIVE2_SIG2_DECORR GIVE2_N_MIN ...
        GIVE2_KA_RATIO GIVE2_SIG2_L1L2REC GIVE2_SIG2_L1L2SAT ...
        GIVE2_GIVEI_FLOOR GIVE2_UNDERSAMP_PTS GIVE2_CHI2_THRESH ...
        GIVE2_MEX_SLOPE1 GIVE2_MEX_SLOPE2 GIVE2_MEX_LLAT1 GIVE2_MEX_LLAT2 ...
        GIVE2_ANT_A1 GIVE2_ANT_B1 GIVE2_ANT_C1 GIVE2_ANT_ALPHA1 ...
        GIVE2_ANT_A2 GIVE2_ANT_B2 GIVE2_ANT_C2 GIVE2_ANT_ALPHA2 ...
        GIVE2_UPMUNSAMP_PTS GIVE2_KHMI_KA
...
if ~isempty(mex_wrs)
    ...
    low_mex_ipps = find(ismember(wrs2satdata(:,COL_U2S_UID),mex_wrs) & ...
                        ...
                        GIVE2_MEX_SLOPE1*wrs2satdata(:,COL_U2S_IPPLL(2)) + ...
                        GIVE2_MEX_LLAT1 | wrs2satdata(:,COL_U2S_IPPLL(1)) < ...
                        GIVE2_MEX_SLOPE2*wrs2satdata(:,COL_U2S_IPPLL(2)) + ...
                        GIVE2_MEX_LLAT2));
...
for idx = 1:length(wrs_s);
    ...
    M(elem,elem) = M(elem,elem) + GIVE2_SIG2_L1L2REC*obl_mat(elem,elem);
end
prn_s=unique(sat_idx);
for idx = 1:length(prn_s);
    elem = find(sat_idx==prn_s(idx));
    M(elem,elem) = M(elem,elem) + GIVE2_SIG2_L1L2SAT*obl_mat(elem,elem);
end
...
mu_me = (CONST_F2^2/(CONST_F1^2-CONST_F2^2))*sqrt(inv_obl2).*...
        (GIVE2_ANT_C1*(GIVE2_ANT_A1 + GIVE2_ANT_B1*...
         exp(-wrs2satdata(abv_mask,COL_U2S_EL)/GIVE2_ANT_ALPHA1)) + ...
         GIVE2_ANT_C2*(GIVE2_ANT_A2 + GIVE2_ANT_B2*...
         exp(-wrs2satdata(abv_mask,COL_U2S_EL)/GIVE2_ANT_ALPHA2)));
...
for igp=1:n_igp
  ...
  [sig2_igp, mu_igp, sig2_upm, r, rcmx, idx, delay, chi2] = ...
                igp_planeR89(xyz_igp(igp,:), igp_en_hat(igp,:), ...
  ...
  if(n_ipp>=GIVE2_N_MIN)
    ...
    if(checkfor2(wrs_idx(idx)))
      ...
      sig2_threat = undersamp_threat(r/1000, rcmx, GIVE2_UNDERSAMP_PTS);
      sig2_upm_threat = undersamp_threat(r/1000, rcmx, GIVE2_UPMUNSAMP_PTS);
      
      %assemble GIVE 
      sig2_give=GIVE2_KA_RATIO^2*((sqrt(sig2_igp) + mu_igp/GIVE2_KHMI_KA)^2 + ...
                                     sig2_threat);
      ...
      if(sig2_give <= MOPS_SIG2_GIVE(MOPS_GIVEI_NM-1))
        ...
        %storm detector
        if chi2 >= GIVE2_CHI2_THRESH(n_ipp-3)
...
%check global floor
below_floor=find(givei < GIVE2_GIVEI_FLOOR);
if(~isempty(below_floor))
  givei(below_floor)=GIVE2_GIVEI_FLOOR;
end
